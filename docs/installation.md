...
The `spec.version` examples throughout these docs use **3.6.x** for consistency, but any supported etcd version works — including on the restore path, which rebuilds the data dir with the `etcdutl` from the target etcd image (`v<spec.version>`) rather than a single bundled one (see the [restore runbook](operations.md#restoring-a-cluster-from-a-snapshot)).
